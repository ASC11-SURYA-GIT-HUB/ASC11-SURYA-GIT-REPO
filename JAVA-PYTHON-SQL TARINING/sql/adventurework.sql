...
SELECT p.Name AS ProductName, AVG(sod.OrderQty) AS AvgQuantity
FROM SalesLT.Product p
JOIN SalesLT.SalesOrderDetail sod ON p.ProductID = sod.ProductID
GROUP BY p.Name
...
SELECT p.Name, p.ListPrice, p.ProductCategoryID
FROM SalesLT.Product p
...
SELECT SalesOrderID, OrderDate, TotalDue,
       AVG(TotalDue) OVER (ORDER BY OrderDate ROWS BETWEEN 2 PRECEDING AND CURRENT ROW) AS MovingAvgLast3Orders
FROM SalesLT.SalesOrderHeader
ORDER BY OrderDate;
...
SELECT 
    ...
    soh.Freight,
    soh.TotalDue,
    -- Additional cost percent = (TaxAmt + Freight) / SubTotal * 100
    CASE 
        WHEN SUM(sod.LineTotal) = 0 THEN 0
        ELSE ((soh.TaxAmt + soh.Freight) / SUM(sod.LineTotal)) * 100 
    END AS AdditionalCostPercent
FROM SalesLT.SalesOrderHeader soh
JOIN SalesLT.Customer c ON soh.CustomerID = c.CustomerID
JOIN SalesLT.SalesOrderDetail sod ON soh.SalesOrderID = sod.SalesOrderID
GROUP BY 
    soh.SalesOrderID, c.FirstName, c.LastName, soh.OrderDate, soh.ShipDate, soh.TaxAmt, soh.Freight, soh.TotalDue
...
SELECT 
    CASE
        ...
        WHEN TotalSpending >= 5000 THEN 'Medium Value'
        ELSE 'Low Value'
    END AS Segment,
...
SELECT 
    ...
    CASE 
        ...
        ELSE COALESCE(SUM(sod.LineTotal), 0) / p.StandardCost
...
SELECT 
    ...
    -- Active products (SellStartDate <= today and (SellEndDate is null or >= today))
    (SELECT COUNT(*) FROM SalesLT.Product WHERE SellStartDate <= GETDATE() AND (SellEndDate IS NULL OR SellEndDate >= GETDATE())) AS ActiveProducts,
    ...
    (SELECT COUNT(*) FROM SalesLT.SalesOrderHeader 
     WHERE YEAR(OrderDate) = YEAR(GETDATE()) AND MONTH(OrderDate) = MONTH(GETDATE())) AS OrdersThisMonth,
